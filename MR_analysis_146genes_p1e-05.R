#!/usr/bin/env Rscript
# MR分析脚本 - 146个基因完整版, eQTLGen p=1e-05, MEGASTROKE结局
# 包含完整敏感性分析: 异质性检验、多效性检验、留一法
...
output_dir    <- "D:/EQTL/mr_results_146genes_p1e-05"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

cat("=" , paste(rep("=", 80), collapse = ""), "\n", sep = "")
cat("       146基因 MR完整分析 (p=1e-05, MEGASTROKE)\n")
cat("=" , paste(rep("=", 80), collapse = ""), "\n", sep = "")
cat("包含: IVW, MR-Egger, Weighted Median, Weighted Mode\n")
cat("敏感性分析: 异质性检验、多效性检验、留一法\n")
...
for (gene in genes) {
  ...
  if (!is.null(hetero) && nrow(hetero) > 0) {
    ...
    if (length(ivw_q) > 0 && !is.na(ivw_q[1])) {
      ...
      cat("    - 异质性检验:", hetero_flag, "(P =", format(ivw_q[1], digits = 3, scientific = TRUE), ")\n")
    }
  }
  ...
  if (!is.null(pleio) && nrow(pleio) > 0) {
    ...
    pleio_flag <- ifelse(pleio_p < 0.05, "⚠️ 定向多效性", "✓ 无多效性")
    cat("    - 多效性检验:", pleio_flag, "(P =", format(pleio_p, digits = 3, scientific = TRUE), ")\n")
  }
  
  # 留一法分析
  loo <- tryCatch(mr_leaveoneout(dat), error = function(e) NULL)
  if (!is.null(loo) && nrow(loo) > 0) {
    cat("    - 留一法: 已执行 (", nrow(loo), "个SNP)\n", sep="")
  }
  ...
  if (nrow(ivw) > 0 && !is.na(ivw$b[1])) {
    ...
    cat("  IVW结果: OR =", round(exp(ivw$b[1]), 3), ", P =", format(ivw$pval[1], digits = 3, scientific = TRUE), sig_marker, "\n")
  }
  cat("\n")
...
if (length(results_list) > 0) {
  ...
  if (nrow(ivw_summary) > 0) {
    ...
    print(ivw_sorted[1:display_n, c("Gene", "Beta", "OR", "P_value", "SNP_n")], row.names = FALSE)
