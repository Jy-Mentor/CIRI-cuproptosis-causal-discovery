...
cat("========== eQTLGen p=1e-05 MR 分析（chr/pos/allele匹配版） ==========\n")
...
exposure_snps <- as.data.frame(exposure_snps)  # 转换为普通data.frame
...
exposure_snps$gene_name <- toupper(as.character(exposure_snps[[gene_col]]))  # 统一转为大写
...
# 使用正确的MEGASTROKE GWAS结局数据
outcome_file <- "D:/EQTL/mr_results_megastroke/megastroke_outcome.csv"

if (!file.exists(outcome_file)) {
  cat("错误: 未找到结局文件:", outcome_file, "\n")
  cat("请先运行Python脚本提取MEGASTROKE数据\n")
  quit(status = 1)
}

cat("使用MEGASTROKE GWAS结局数据...\n")
...
for (gene in genes) {
  ...
  # 去重：保留每个chr:pos中pval最小的一条（防止笛卡尔积放大样本量）
  ...
  # 同步翻转等位基因（重要修复！）
  merged_clean$effect_allele.out.flip <- with(merged_clean,
    ifelse(effect_allele.exp == effect_allele.out, effect_allele.out, other_allele.out)
  )
  
  merged_clean$other_allele.out.flip <- with(merged_clean,
    ifelse(effect_allele.exp == effect_allele.out, other_allele.out, effect_allele.out)
  )
  ...
  dat <- data.frame(
    ...
    effect_allele.outcome = merged_clean$effect_allele.out.flip,  # 使用翻转后的
    other_allele.outcome = merged_clean$other_allele.out.flip,   # 使用翻转后的
