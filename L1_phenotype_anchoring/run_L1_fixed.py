...
# 修复：添加 filter_cells/filter_genes + Harmony 批次校正 + score_genes 注释
import scanpy as sc
import anndata
import numpy as np
import pandas as pd
...
CUPROPTOSIS_CU_HOMEOSTASIS = ['Slc31a2', 'Slc11a2', 'Steap3', 'Atox1', 'Ccs', 'Cox11',
                               'Sco1', 'Sco2', 'Mt1', 'Mt2', 'Alb', 'Cp', 'Sod1', 'Sod3',
                               'Commd1']
CUPROPTOSIS_ALL = list(dict.fromkeys(CUPROPTOSIS_CORE + CUPROPTOSIS_CU_HOMEOSTASIS))
...
def quality_control(adata):
    print("\n=== 步骤3: 质量控制（含 filter_cells/genes）===")
    adata.var["mt"] = adata.var_names.str.startswith("mt-")
    adata.var["ribo"] = adata.var_names.str.startswith(("Rps", "Rpl"))
    sc.pp.calculate_qc_metrics(adata, qc_vars=["mt", "ribo"], inplace=True)
    print(f"  质控前: {adata.n_obs} cells, {adata.n_vars} genes")
    # 过滤低质量细胞和基因
...
def normalize_and_cluster(adata):
    ...
    try:
        ...
        adata.obsm["X_pca_harmony"] = harmonypy.run_harmony(
            adata.obsm["X_pca"], adata.obs, batch_key="sample_id"
        ).Z_corr.T
        use_pca = "X_pca_harmony"
        print("  Harmony 校正完成")
    except ImportError:
        print("  harmonypy 未安装，使用原始 PCA")
        use_pca = "X_pca"
    sc.pp.neighbors(adata, random_state=SEED, use_rep=use_pca)
...
def differential_expression(adata):
    print("\n=== 步骤6: 差异表达分析 ===")
    sc.tl.rank_genes_groups(adata, groupby="condition", reference="Sham", method="wilcoxon", use_raw=True)
