#!/usr/bin/env Rscript
# ================================================================================
# 138 基因 MR 分析（双源 eQTL 整合 - 修复版）
# 参考 GitHub 权威实践：使用染色体位置匹配而非 SNP ID
# - eQTLGen: 已 LD clumped (p<5e-8, kb=10000, r²=0.001)
# - GTEx 脑组织：所有显著 eQTL (p<5e-8, 无需 clumping)
# - 匹配策略：chr:pos 匹配（解决 SNP ID 格式不一致问题）
# ================================================================================
...
cat("======================================================================\n")
cat("138 基因 MR 分析（双源 eQTL 整合 - 染色体位置匹配修复版）\n")
cat("======================================================================\n\n")

# 配置
CLUMPED_FILE <- "D:/EQTL/clump/eQTLgen_allgene_p_5e-8_kb_10000_r2_0.001.xlsx"
...
OUTPUT_DIR <- "C:/Users/Jy-Mentor-7/Desktop/你信我 这不是重蹈覆辙/mr_results_dual_source_fixed"
...
# 处理 MEGASTROKE 数据 - 创建 chr:pos 键
# 注意：MEGASTROKE 使用 GRCh37 坐标，GTEx v11 使用 GRCh38，需要 liftover 或接受近似匹配
outcome_data$chr <- as.character(outcome_data$hm_chrom)
outcome_data$pos <- outcome_data$hm_pos
outcome_data$rsid <- outcome_data$hm_rsid
outcome_data$beta <- outcome_data$hm_beta
outcome_data$se <- outcome_data$hm_beta  # 近似处理，实际应该用 SE
outcome_data$effect_allele <- outcome_data$hm_effect_allele
outcome_data$other_allele <- outcome_data$hm_other_allele
outcome_data$eaf <- outcome_data$hm_effect_allele_frequency
outcome_data$pval <- 1  # 占位符
outcome_data$chr_pos <- paste(outcome_data$chr, outcome_data$pos, sep = ":")
...
for (gene_symbol in target_genes) {
  ...
  if (nrow(eqtlgen_snps) > 0) {
    eqtlgen_df <- data.frame(
      ...
      CHR = as.character(eqtlgen_snps$chr.exposure),
    ...
    if (nrow(gtex_snps) > 0) {
      ...
      gtex_df <- data.frame(
        ...
        CHR = gsub("chr", "", sapply(strsplit(gtex_snps$variant_id, "_"), function(x) x[1])),
...
for (gene_symbol in names(prepared_genes)) {
  ...
  tryCatch({
    ...
    region_data <- outcome_data[outcome_data$chr == chr & 
                                outcome_data$pos >= min_bp & 
                                outcome_data$pos <= max_bp, ]
    ...
    if (length(matched_chr_pos) < 2) {
      cat(sprintf("  ✗ 匹配的 chr:pos 太少 (%d)\n", length(matched_chr_pos)))
      next
    }
    
    cat(sprintf("  匹配的 chr:pos：%d\n", length(matched_chr_pos)))
    ...
    # 手动创建 TwoSampleMR 格式的数据
    # 因为 GTEx 的 SNP ID 格式与 MEGASTROKE 不同，需要用 rsID
    dat <- data.frame(
      SNP = out_matched$rsid,
      ...
      eaf.outcome = out_matched$eaf,
      pval.outcome = out_matched$pval,
      samplesize.outcome = nrow(outcome_data),
      stringsAsFactors = FALSE
    )
    
    # 等位基因匹配
    allele_match <- (dat$effect_allele.exposure == dat$effect_allele.outcome) |
                    (dat$effect_allele.exposure == dat$other_allele.outcome)
    
    cat(sprintf("  等位基因匹配：%d/%d\n", sum(allele_match), nrow(dat)))
    
    if (sum(allele_match) < 2) {
      cat(sprintf("  ✗ 等位基因匹配的 SNP 太少\n"))
      next
    }
    
    dat <- dat[allele_match, ]
    
    # 翻转 beta（如果需要）
    need_flip <- dat$effect_allele.exposure != dat$effect_allele.outcome
    if (any(need_flip)) {
      dat$beta.exposure[need_flip] <- -dat$beta.exposure[need_flip]
    }
...
if (length(mr_results) > 0) {
  ...
  sink(quality_report)
  cat("MR 分析质量控制报告（双源 eQTL - 染色体位置匹配）\n")
  cat("===============================================\n\n")
  cat("1. F 统计量评估\n")
