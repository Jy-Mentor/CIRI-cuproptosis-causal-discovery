...
brain_genes_df <- fread(GTEx_BRAIN_EGENES, sep="\t")
blood_genes_df <- fread(GTEx_BLOOD_EGENES, sep="\t")
...
# 保存统计文件
writeLines(sprintf("总基因数：%d\n成功处理：%d\n\n组织分布:\n  - 双组织都有：%d\n  - 仅脑组织：%d\n  - 仅全血：%d\n  - 都无数据：%d\n",
                   length(target_genes), processed, stats$both, stats$brain_only, stats$blood_only, stats$neither),
           file.path(OUTPUT_DIR, "stats.txt"))
