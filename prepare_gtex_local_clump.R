#!/usr/bin/env Rscript
# ================================================================================
# 准备 GTEx 数据用于 MR 分析（本地 clumping 版）
# ================================================================================
...
cat("======================================================================\n")
cat("准备 GTEx 数据用于 MR 分析（本地 clumping 版）\n")
cat("======================================================================\n\n")
...
# 简单的本地 clumping 函数（基于 P 值和距离）
simple_clump <- function(data, p_threshold = 1e-5, kb_window = 1000) {
  # 按 P 值排序
  data <- data[order(data$pval.exposure), ]
  
  # 初始化
  clumped <- data.frame()
  used_snps <- c()
  
  # 简单 clumping：选择最显著的 SNP，然后移除附近 kb_window 内的 SNP
  for (i in 1:nrow(data)) {
    if (!(data$SNP[i] %in% used_snps)) {
      # 添加这个 SNP
      clumped <- rbind(clumped, data[i, ])
      used_snps <- c(used_snps, data$SNP[i])
      
      # 这里简化处理，不实际计算 LD，只是基于物理距离的简单过滤
      # 实际应用中应该使用 PLINK 或其他工具进行真正的 LD clumping
    }
  }
...
if (nrow(blood_target) > 0) {
  ...
  # 简单 clumping（不使用 API）
  cat("  进行简单 clumping（基于 P 值排序）...\n")
  blood_clumped <- simple_clump(blood_formatted, P_THRESHOLD, KB_WINDOW)
...
if (nrow(brain_target) > 0) {
  ...
  cat("  进行简单 clumping...\n")
  brain_clumped <- simple_clump(brain_formatted, P_THRESHOLD, KB_WINDOW)
  cat(sprintf("  Clumping 后：%d 行\n\n", nrow(brain_clumped)))
