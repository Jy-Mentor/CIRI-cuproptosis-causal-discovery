#!/usr/bin/env Rscript
# MR分析脚本 - 40个基因完整版, eQTLGen p=1e-05, MEGASTROKE结局
# 包含MR-PRESSO分析
...
# 检查MR-PRESSO是否可用
has_presso <- require(MRPRESSO, quietly = TRUE)
if (!has_presso) {
  cat("警告: MR-PRESSO包未安装，将跳过MR-PRESSO分析\n")
  cat("如需安装请运行: install.packages(\"MRPRESSO\")\n\n")
}

# 40个基因列表
genes <- c("NFKB1", "RELA", "IKBKB", "STAT3", "STAT1", "JAK1", "IRF1", "TNF", "IL6", "TGFB1",
...
outcome_file  <- "D:/EQTL/mr_results_megastroke/megastroke_outcome_40genes.csv"
output_dir    <- "D:/EQTL/mr_results_40genes_p1e-05_complete"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

cat("========== 40基因 MR完整分析 (p=1e-05, MEGASTROKE) ==========\n")
cat("包含: IVW, MR-Egger, Weighted Median, Weighted Mode, MR-PRESSO\n")
...
for (gene in genes) {
  ...
  if (nrow(dat) > 0) {
    cat("  F>10 SNP:", nrow(dat), "个 (F范围:", round(min(dat$F_stat), 1), "-", round(max(dat$F_stat), 1), ")\n")
  } else {
    cat("  F>10 SNP: 0 个\n")
    next
  }
  
  # 主分析：IVW, MR-Egger, Weighted Median, Weighted Mode
  res <- tryCatch({
  ...
  if (has_presso && nrow(dat) >= 4) {
    presso_result <- tryCatch({
      cat("  运行MR-PRESSO...\n")
      mr_presso(BetaOutcome = dat$beta.outcome, BetaExposure = dat$beta.exposure,
                SdOutcome = dat$se.outcome, SdExposure = dat$se.exposure,
  ...
  results_list[[gene]] <- list(gene = gene, dat = dat, res = res, 
                               hetero = hetero, pleio = pleio, loo = loo,
  ...
  if (!is.null(presso_result)) {
    rssobs <- presso_result$`MR-PRESSO results`$`Global Test`$RSSobs
    pval <- presso_result$`MR-PRESSO results`$`Global Test`$Pvalue
    cat("  MR-PRESSO全局检验: RSSobs=", round(rssobs, 2), ", P=", format(pval, digits = 3, scientific = TRUE), "\n", sep="")
  }
  cat("\n")
  ...
  if (!is.null(presso_result)) saveRDS(presso_result, file.path(output_dir, paste0(gene, "_presso.rds")))
}
...
if (length(results_list) > 0) {
  summary_df <- data.frame(
    ...
    OR = numeric(), P_value = numeric(), F_stat_mean = numeric(), SNP_n = integer(),
  ...
  for (gene in names(results_list)) {
    ...
    for (i in seq_len(nrow(res))) {
      summary_df <- rbind(summary_df, data.frame(
        ...
        OR = exp(res$b[i]), P_value = res$pval[i], F_stat_mean = mean(dat$F_stat), SNP_n = nrow(dat)
      ))
    }
  }
  ...
  if (nrow(ivw_summary) > 0) {
    ...
    print(ivw_sorted[, c("Gene", "Beta", "OR", "P_value", "SNP_n")], row.names = FALSE)
  }
