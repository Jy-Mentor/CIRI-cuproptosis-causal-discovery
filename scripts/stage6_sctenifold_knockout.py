...
原理: 基于共表达网络的基因扰动分析 (CellOracle/scTenifoldKnk方法)
  1. 计算基因间Spearman相关系数构建共表达网络
  2. 虚拟敲除目标基因（表达设为0）
  3. 根据回归系数预测下游基因表达变化
  4. 使用t分布近似估计显著性 (替代1000次置换)
...
注意:
  - 本阶段使用CellOracle/scTenifoldKnk方法，与GraphSAGE无关
  - 原文件名stage6_graphsage_knockout.py为历史遗留错误命名
...
STAGE_DIR = os.path.join(RESULTS_DIR, "stage6_sctenifold_knockout")
...
def virtual_knockout(corr_matrix, expr, gene_names, target_gene):
    ...
    # 低表达基因跳过 (使用变异系数判断)
    cv = float(np.std(target_expr) / (abs(mean_expr) + 1e-10))
    if cv < 0.1:
...
def main():
    ...
    logger.info(f"方法: Spearman相关 + t分布近似 + Cohen's d效应大小")
    logger.info(f"参考: CellOracle (PMID: 368363447)")
    logger.info("注意: 本阶段使用CellOracle/scTenifoldKnk方法，与GraphSAGE无关")
