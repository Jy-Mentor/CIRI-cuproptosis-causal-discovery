# -*- coding: utf-8 -*-
"""
GAT Embedding Extractor v7 — L2-Normalized PCA + Improved Model B Convergence
  v6 problem: PCA amplified raw norm gap (2x → 19x), PCA-space vector norm
  itself became a perfect label predictor (AUC=1.0). Single-PC AUCs < 0.8
  but combined 12-dim magnitude leaked. Model B also converged poorly
  (loss 0.087→0.056 only, 35% reduction).

  v7 fix:
    - PCA → L2 normalize each gene's 12-dim vector → unit sphere
    - All genes have ‖embedding‖ = 1.0 → magnitude can NOT predict label
    - RF must use directional (angular) information only
    - Model B: EPOCHS 600→1200 + ReduceLROnPlateau scheduler

  Design principle:
    - Positives (DT/DG genes): OOF from their validation fold only
    - Negatives (non-DT/non-DG genes): MEAN across all 5 OOF folds
    - NO full model used anywhere
    - L2 norm removes amplitude → forces RF to learn geometry
...
RF_DIR = os.path.join(SCRIPT_DIR, "gat_output")
FEATURE_TABLE = os.path.join(SCRIPT_DIR, "gene_features_table_with_gat.csv")
...
def train_model(model, data, dt_edges, td_edges, dt_sampler, td_sampler,
                n_genes, neg_ratio, epochs, patience, model_name, use_scheduler=False):
    opt = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        opt, mode="min", factor=SCHEDULER_FACTOR, patience=SCHEDULER_PATIENCE,
        min_lr=1e-6) if use_scheduler else None
    ...
    for ep in range(epochs):
        ...
        if ep % 100 == 0:
            ...
            print(f"  E{ep:4d}: loss={loss_val:.4f}  DT={loss_dt.item():.4f}  TD={loss_td.item():.4f}  lr={lr_now:.2e}")
...
def main():
    print("=" * 60)
    print("  GAT Embedding v7 — L2-Normalized PCA + Improved Model B")
    print("  PCA → L2 normalize → all genes on unit sphere")
    print("  Model B: 1200 epochs + ReduceLROnPlateau scheduler")
    print("  → Magnitude can NOT leak → RF uses direction only")
    print("=" * 60)
    ...
    # ════════════════════════════════════════════════════
    #  L2 NORMALIZE — eliminate amplitude as a signal
    #  All genes now live on the unit sphere in 12-dim
    #  PCA norm can NO LONGER predict labels
    # ════════════════════════════════════════════════════
    print("\n" + "=" * 60)
    print("  L2 NORMALIZATION")
    print("=" * 60)
    norms_a_before = np.linalg.norm(emb_a_pca, axis=1)
    norms_b_before = np.linalg.norm(emb_b_pca, axis=1)
    eps = 1e-8
    emb_a_pca = emb_a_pca / (np.linalg.norm(emb_a_pca, axis=1, keepdims=True) + eps)
    emb_b_pca = emb_b_pca / (np.linalg.norm(emb_b_pca, axis=1, keepdims=True) + eps)
    norms_a_after = np.linalg.norm(emb_a_pca, axis=1)
    norms_b_after = np.linalg.norm(emb_b_pca, axis=1)
    print(f"  [A] Norm before L2: mean={norms_a_before.mean():.4f} std={norms_a_before.std():.4f}  "
          f"min={norms_a_before.min():.6f} max={norms_a_before.max():.4f}")
    print(f"  [A] Norm after  L2: mean={norms_a_after.mean():.4f} std={norms_a_after.std():.4f}  "
          f"min={norms_a_after.min():.6f} max={norms_a_after.max():.4f}")
    print(f"  [B] Norm before L2: mean={norms_b_before.mean():.4f} std={norms_b_before.std():.4f}  "
          f"min={norms_b_before.min():.6f} max={norms_b_before.max():.4f}")
    print(f"  [B] Norm after  L2: mean={norms_b_after.mean():.4f} std={norms_b_after.std():.4f}  "
          f"min={norms_b_after.min():.6f} max={norms_b_after.max():.4f}")
    dead_a = (norms_a_before < 1e-8).sum()
    dead_b = (norms_b_before < 1e-8).sum()
    if dead_a > 0:
        print(f"  [A] {dead_a} zero-norm genes → stay zero after L2")
    if dead_b > 0:
        print(f"  [B] {dead_b} zero-norm genes → stay zero after L2")
    print(f"  ✓ All non-zero genes now have ‖embedding‖ ≈ 1.0")
    ...
    # Fix: zero-norm genes (from fillna) → random unit vector to avoid norm leak
    rng = np.random.RandomState(RANDOM_SEED)
    for cols_set in [cols_a, cols_b]:
        X = merged[cols_set].values
        norms = np.linalg.norm(X, axis=1)
        zero_mask = norms < 1e-8
        n_zero = zero_mask.sum()
        if n_zero > 0:
            rand_vecs = rng.randn(n_zero, len(cols_set)).astype(np.float64)
            rand_vecs = rand_vecs / (np.linalg.norm(rand_vecs, axis=1, keepdims=True) + 1e-8)
            X[zero_mask] = rand_vecs
            merged[cols_set] = X
            print(f"  [Fix] {n_zero} zero-norm genes in {cols_set[0][:-2]} → random unit vectors")

    # Re-normalize ALL genes to unit norm before CSV save
    for cols_set in [cols_a, cols_b]:
        X = merged[cols_set].values.astype(np.float64)
        norms = np.linalg.norm(X, axis=1, keepdims=True)
        norms = np.where(norms < 1e-8, 1e-8, norms)
        merged[cols_set] = X / norms

    out_path = os.path.join(RF_DIR, "gene_features_table_with_gat_emb.csv")
    merged.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"\n[Save] → {out_path}  ({merged.shape[0]} x {merged.shape[1]})")

    # Post-save: reload & re-normalize to fix CSV float64 precision loss
    merged2 = pd.read_csv(out_path)
    for cols_set in [cols_a, cols_b]:
        X = merged2[cols_set].values.astype(np.float64)
        norms = np.linalg.norm(X, axis=1, keepdims=True)
        norms = np.where(norms < 1e-8, 1e-8, norms)
        merged2[cols_set] = X / norms
    merged2.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"[Post-save] Re-normalized to fix CSV precision loss")

    # ════════════════════════════════════════════════════
    #  Leakage Check (L2-normalized)
    # ════════════════════════════════════════════════════
    print("\n[Leak Check] v7 L2-Normalized:")
    print("  PCA vector norm should be ~1.0 for all genes → AUC(norm, label) ≈ 0.5")
    from sklearn.metrics import roc_auc_score
    df_chk = pd.read_csv(out_path)
    y_dt = df_chk["is_drug_target"].values
    y_dg = df_chk["is_disease_gene"].values

    # First check: PCA norm leak
    norm_a = np.linalg.norm(df_chk[cols_a].values, axis=1)
    norm_b = np.linalg.norm(df_chk[cols_b].values, axis=1)
    auc_norm_a_dt = roc_auc_score(y_dt, norm_a)
    auc_norm_b_dg = roc_auc_score(y_dg, norm_b)
    if auc_norm_a_dt < 0.5: auc_norm_a_dt = roc_auc_score(y_dt, -norm_a)
    if auc_norm_b_dg < 0.5: auc_norm_b_dg = roc_auc_score(y_dg, -norm_b)
    print(f"  AUC(norm_gat_nodg, DT) = {auc_norm_a_dt:.4f}  {'⚠️ LEAK' if auc_norm_a_dt > 0.80 else '✓ CLEAN'}")
    print(f"  AUC(norm_gat_nodt, DG) = {auc_norm_b_dg:.4f}  {'⚠️ LEAK' if auc_norm_b_dg > 0.80 else '✓ CLEAN'}")

    # Second check: per-dimension AUC
    ...
    if max_auc["DT"] < 0.90 and max_auc["DG"] < 0.90 and auc_norm_a_dt < 0.80 and auc_norm_b_dg < 0.80:
        print("  ✓ No leakage: single-dim + PCA norm all clean")
    else:
        print("  ⚠️ Some dimensions or norm still show high AUC — check above")

    print("\n[Done] GAT v7 L2-Normalized embeddings — unit sphere, magnitude-safe.")
