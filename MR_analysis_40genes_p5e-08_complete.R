#!/usr/bin/env Rscript
# MR分析脚本 - 40个基因完整版, eQTLGen p=5e-08, MEGASTROKE结局
...
output_dir    <- "D:/EQTL/mr_results_40genes_p5e-08_complete"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

cat("========== 40基因 MR完整分析 (p=5e-08, MEGASTROKE) ==========\n")
...
for (gene in genes) {
  ...
  cat("  去重后:", nrow(exp_df), "个暴露SNP,", nrow(out_df), "个结局SNP\n")
