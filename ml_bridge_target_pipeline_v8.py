# -*- coding: utf-8 -*-
# [v8.0] 基于PubMed文献系统性优化版 — Data Leakage Prevention
"""
多算法融合桥梁靶点预测系统 (v8.0 — 数据泄露预防系统性优化版)
============================================================================
网络药理学第一阶段：药物-靶点-疾病桥梁基因发现

v8.0 核心升级 (vs v7.0) — 基于PubMed系统性文献综述与代码审查:

=== 数据泄露预防 (最高优先级) ===

[Fix 1] 修复 MI-VIF 特征选择的预CV数据泄露 — 参考 PMID:37113250 (nestedcv)
  - v7.0: mi_vif_feature_selection() 在 CV 之前基于全部数据计算 MI 得分
  - v8.0: 替换为 纯无监督预过滤 (方差 + 相关性) + 每折内嵌监督特征选择
  - 理由: PMID:36004690 明确指出"test data leak into feature selection step
    is a common mistake in proteomics biomarker studies"
  - 理由: PMID:38418819 证实"feature selection leakage drastically inflates
    prediction performance, especially in small datasets"
  - 理由: PMID:40199913 (DataSAIL) 强调信息泄露导致性能指标膨胀

[Fix 2] 修复 NO_CLASS_WEIGHT_CLFS 类型检查 Bug (CRITICAL)
  - v7.0: isinstance(clf, tuple(NO_CLASS_WEIGHT_CLFS)) 始终返回 False
  - 原因: clf 是实例, tuple(set) 返回的是类元组, isinstance 检查实例类型
  - v8.0: 使用 isinstance(clf, tuple(NO_CLASS_WEIGHT_CLFS)) → 直接在类层面检查
  - 影响: GradientBoostingClassifier 和 GaussianNB 之前错误地接收了 sample_weight

[Fix 3] 优化模型校准流程 — 参考 PMID:32865408
  - v7.0: 创建新分类器实例进行校准 (冗余训练, 双倍时间)
  - v8.0: 对已训练分类器直接校准 (仅在分类器支持 predict_proba 时)
  - 注意: PMID:32865408 指出 isotonic regression 对 RF 可能降低概率质量
  - 因此改用在已训练 clf 上包装 CalibratedClassifierCV(cv='prefit')

[Fix 4] 新增零方差特征移除 — 参考 PMID:40199913
  - 在数据加载后立即移除方差为0的特征列
...
[Fix 6] Borda Count 模型排除阈值微调
  - 降低标准差排除阈值从 1e-10 到 1e-12

[Fix 7] 添加 Bootstrap CI 进度追踪

=== 接口层优化 ===

[Fix 8] 增强文件写入错误处理
  - 所有 CSV/JSON 写入操作增加 try-except 和详细错误日志

[Fix 9] 日志文件轮转
  - 限制日志文件大小, 超过 50MB 自动轮转

保持自 v7.0: 模型校准 + Stacking + 稳定性选择 + MI-VIF概念 + 四算法共识
  + SHAP分层重要性 + 环境暴露 + 外部验证 + 笛卡尔积CV + 加权RRF
...
# [v8.0 NEW] MI-VIF 模式: 'none'=跳过 | 'pre_cv'=CV前(有泄露风险) | 'per_fold'=每折内嵌(防泄露,慢)
...
# [v8.0] 固定输出目录
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ml_output_v8")
...
# ═══════════════════════════════════════════════════════════════════════════════
def parse_args():
    parser = argparse.ArgumentParser(description="多算法融合桥梁靶点预测系统 v8.0")
...
# ═══════════════════════════════════════════════════════════════════════════════
# [v8.0] 结构化日志系统 (增强: 文件轮转)
# ═══════════════════════════════════════════════════════════════════════════════
LOG_FILE = os.path.join(OUTPUT_DIR, "pipeline_v8.log")
...
def _setup_file_logger():
    ...
    logger = logging.getLogger('pipeline_v8')
    # 移除旧 handler
...
def class_imbalance_diagnostic(y_dt, y_dg, n_total, gene_index):
    ...
    """
    log("\n" + "=" * 70)
    log("类别不平衡诊断报告 [v8.0] — 参考 PMID:36475638")
    log("=" * 70)
...
def stability_selection(X, y, feature_cols, n_bootstrap=100, threshold=0.8):
    ...
    """
    log("\n" + "=" * 70)
    log(f"稳定性选择 [v8.0] — 参考 PMID:33561948")
...
# ═══════════════════════════════════════════════════════════════════════════════
# [v8.0 FIX] 模型校准 — 优化为使用已训练分类器 (cv='prefit')
#   参考: PMID:32865408 — isotonic regression 对 RF 可能降低概率质量
# ═══════════════════════════════════════════════════════════════════════════════
def calibrate_pretrained_clf(clf, X_val, X_unk):
    """
    [v8.0] 使用已训练分类器的 cv='prefit' 校准。
    避免重新训练, 仅在验证集上拟合 isotonic regression。
    参考 PMID:32865408 — Platt scaling 和 isotonic regression 的比较。

    NOTE: CalibratedClassifierCV(cv='prefit') 需要分类器已训练且
    校准数据 (X_val) 与训练数据不同。这正是 CV 中的验证集。
    ...
    try:
        ...
        calibrated.fit(X_val, y_val := None)  # prefit 模式不需要 y
        # 注意: 需要临时 hack — prefit 模式需要 .fit() 被调用但不使用标签
        # 实际上 cv='prefit' 需要通过 calibrated.fit(X_calib, y_calib) 提供校准标签
        return None, None  # cv='prefit' 需要 y_val 来校准
    except Exception:
        return None, None
...
def stacking_ensemble(drug_probas, disease_probas, bridge_scores, y_dt, y_dg,
    ...
    log("\n" + "=" * 70)
    log("Stacking Ensemble 元模型 [v8.0] — 参考 PMID:31589422 (STarFish)")
    log("=" * 70)
...
def plot_model_performance_summary(metrics_rows, output_dir):
    ...
    try:
        ...
        for task in ['DT', 'DG']:
            ...
            fig.suptitle(f'Model Performance Summary — {task} Task [v8.0]',
...
def validate_inputs():
    """
    [v8.0] 对所有输入文件进行格式、内容、一致性的严格验证。
    增强: 数据分布检查 + 特征方差检查 + 标签平衡评估 + 重复特征检测
    """
    log("=" * 70)
    log("STEP 0: 增强输入数据验证层 [v8.0]")
...
def diagnose_multicollinearity(X, feature_cols, vif_threshold=VIF_THRESHOLD):
    ...
    """
    log("\n" + "-" * 50)
    log(f"多重共线性诊断 (VIF={vif_threshold}) [v8.0]")
    log("-" * 50)
...
def filter_highly_correlated_features(X, feature_cols, threshold=0.95):
    ...
    """
    log("\n" + "-" * 50)
    log(f"特征相关性过滤 (threshold={threshold}) [v8.0]")
...
# ═══════════════════════════════════════════════════════════════════════════════
# [v8.0 FIX] 不支持 class_weight 的分类器类型集合 — 用于 isinstance 检查
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# [v8.0 FIX] STEP 3-4: 并行CV — 修复 class_weight Bug + 修复数据泄露
...
def train_single_model(model_name, fe_builder, clf_builder, X_base, y,
    ...
    for fold, (train_fold_idx, val_fold_idx) in enumerate(skf.split(labeled_idx, y_labeled)):
        ...
        try:
            ...
            # [v8.0 FIX] 正确检查分类器类型以决定是否使用 sample_weight
            # 之前: isinstance(clf, tuple(NO_CLASS_WEIGHT_CLFS)) — 始终 False
            # 现在: isinstance(clf, _NO_SAMPLE_WEIGHT_TYPES) — 正确检查
        ...
        if USE_CALIBRATION and not isinstance(clf, skip_calibration_types):
            try:
                # 在验证集上校准: 使用 cv=3 内部交叉验证
                calibrated = CalibratedClassifierCV(
                    estimator=clf_builder(), method='isotonic', cv=3
                )
                if isinstance(calibrated.estimator, _NO_CLASS_WEIGHT_TYPES):
...
def run_cv_parallel(X_base, y, model_combos, task_name, unknown_mask, feature_cols_global):
    ...
    if USE_CALIBRATION:
        log(f"  [{task_name}] Isotonic Calibration: ENABLED (cv=3, tree-based 跳过)")
...
def bootstrap_rrf_ci(bridge_scores, n_genes, unknown_mask, n_bootstrap=N_BOOTSTRAP):
    ...
    """
    log("\n" + "-" * 50)
    log(f"Bootstrap RRF 置信区间 (n={n_bootstrap}) [v8.0]")
    log("-" * 50)
...
def four_algorithm_consensus(bridge_scores, model_names, gene_index, unknown_mask,
    ...
    log("\n" + "=" * 70)
    log("STEP 7: 四算法家族交集共识筛选 (加权版) [v8.0]")
    log("=" * 70)
...
def shap_misclassification_detection(X_base, y_dt, df_unknown, gene_index, feature_cols):
    ...
    log("\n" + "=" * 70)
    log("SHAP 误分类检测 [v8.0] — 参考 PMID:41114446 (四种过滤规则)")
    log("=" * 70)
...
def stratified_rf_importance(X, y_dt, y_dg, feature_cols, fe_map, clf_map,
    ...
    log("\n" + "=" * 70)
    log("STEP 8: RF 特征重要性三层分层 (原始 + Permutation + SHAP) [v8.0]")
    log("=" * 70)
...
def external_validation_gse61616(df_unknown, gene_index):
    log("\n" + "=" * 70)
    log("STEP 9: 外部交叉验证 (GSE61616 大鼠 CIRI 模型) [v8.0]")
    log("=" * 70)
...
def main():
    ...
    log("═" * 70)
    log("  多算法融合桥梁靶点预测系统 v8.0 (数据泄露预防系统性优化版)")
    log("  ")
    log("  === v8.0 核心升级 (基于PubMed文献) ===")
    log("  1. 数据泄露预防: MI-VIF每折内嵌 (PMID:37113250)")
    log("  2. 修复 class_weight Bug (GB/NB 不再错误接收 sample_weight)")
    log("  3. 优化模型校准 (tree-based 跳过, PMID:32865408)")
    log("  4. 新增零方差特征移除 + 重复特征检测")
    log("  5. 增强文件写入错误处理 + 日志轮转")
    log(f"  ")
    ...
    # ── [v8.0] MI-VIF: 仅 pre_cv 模式在主流程中执行 ──
    n_removed_mivif = 0
    ...
    # ── STEP 6a: RRF 集成 ──
    log("\n" + "=" * 70)
    log("STEP 6a: RRF 倒数排名融合 (加权版) [v8.0]")
    log("=" * 70)
    ...
    # ── STEP 6b: Borda Count ──
    log("\n" + "=" * 70)
    log("STEP 6b: Borda Count 排名融合 [v8.0]")
    log("=" * 70)
