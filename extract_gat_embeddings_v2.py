# -*- coding: utf-8 -*-
"""
GAT Embedding Extractor v2 — Dual Model (No Label Leakage)
  ...
  All genes get embeddings from unified models, no OOF asymmetry.
"""
...
def main():
    print("="*60)
    print("  GAT Embedding v2 — Dual Blind Model")
    ...
    print("\n"+"="*50)
    print("  MODEL A: Trained WITHOUT TD (disease-gene) edges")
    print("  → Embeddings for DG classifier (blind to disease labels)")
    print("="*50)
    ...
    dt_all=torch.arange(full_dt.size(1))
    ...
    train_model(model_a,data_a,full_dt,None,dt_sampler_a,None,n_genes)
    ...
    print("\n"+"="*50)
    print("  MODEL B: Trained WITHOUT DT (drug-target) edges")
    print("  → Embeddings for DT classifier (blind to drug labels)")
    print("="*50)
    ...
    td_all=torch.arange(full_td.size(1))
    ...
    train_model(model_b,data_b,None,full_td,None,td_sampler_b,n_genes)
    ...
    print(f"\n[PCA-A] no-TD: 64d→{N_PCA}d, var={var_a:.3f}")
    ...
    print(f"[PCA-B] no-DT: 64d→{N_PCA}d, var={var_b:.3f}")
    ...
    print("\n[Done] Dual blind GAT embeddings ready.")
