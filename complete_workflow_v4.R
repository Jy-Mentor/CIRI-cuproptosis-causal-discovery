...
# 检查哪些基因存在
available_markers <- lapply(marker_genes, function(genes) {
  genes[genes %in% rownames(sc_obj)]
})
...
for(cell_type in names(available_markers)) {
  ...
  if(length(genes) > 0) {
    scores <- colMeans(expr[genes, , drop=FALSE])
    sc_obj[[paste0("score_", cell_type)]] <- scores
  }
}
...
if(length(score_cols) > 0) {
  ...
  sc_obj$cell_type <- gsub("score_", "", colnames(scores_matrix)[apply(scores_matrix, 1, which.max)])
}
