...
def build_seed_genes():
    ...
    # Add known CIRI-related genes (human symbols)
    ciri_genes = [
...
def fetch_ppi_from_string(gene_list):
    ...
    url_base = (
        ...
        f"?identifiers={'%0d'.join(gene_list)}"
        f"&species={STRING_SPECIES}"
        f"&limit=1000000"
    )
    
    # FIX:[P0-3][3-retry exponential backoff]
    for attempt in range(MAX_RETRIES):
        try:
            logger.info(f"  Attempt {attempt + 1}/{MAX_RETRIES}...")
            ppi_df = pd.read_csv(url_base, sep='\t')
            logger.info(f"  STRING API returned: {len(ppi_df)} interactions")
            return ppi_df
    ...
    logger.error(f"  STRING API failed after {MAX_RETRIES} attempts")
    return None
...
def load_local_ppi():
    ...
    # FIX:[P0-3][validate cache freshness]
...
def main():
    logger.info("=" * 60)
    logger.info("Stage 5: STRING PPI Network Construction")
    logger.info("=" * 60)
    ...
    if ppi_df is None:
        logger.info("  Falling back to local cached PPI...")
