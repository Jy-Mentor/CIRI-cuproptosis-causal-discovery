...
try:
    ...
    print("✓ 使用优化的 pyarrow 流式处理\n")
except ImportError as e:
    print(f"✗ 错误：{e}")
    print("\n请运行：setup_env.bat 创建虚拟环境")
    sys.exit(1)
...
brain_genes = set()
with open(GTEx_BRAIN_EGENES, 'r') as f:
    reader = pd.read_csv(f, sep='\t', usecols=['gene_id'])
    brain_genes = set(reader['gene_id'].unique())

blood_genes = set()
with open(GTEx_BLOOD_EGENES, 'r') as f:
    reader = pd.read_csv(f, sep='\t', usecols=['gene_id'])
    blood_genes = set(reader['gene_id'].unique())

print(f"  ✓ 脑皮层：{len(brain_genes):,} 基因")
print(f"  ✓ 全血：{len(blood_genes):,} 基因")
print(f"  ✓ 共同：{len(brain_genes & blood_genes):,} 基因\n")

target_genes = brain_genes | blood_genes
print(f"目标总数：{len(target_genes):,}\n")
