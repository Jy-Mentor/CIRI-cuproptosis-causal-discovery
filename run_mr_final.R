...
# ================================================================================
# 138 基因 MR 分析（最终修复版）
# 参考 GitHub 权威实践和 TwoSampleMR 官方文档
# - 使用 rsID 匹配（而不是 chr:pos）
# - 使用 TwoSampleMR::harmonise_data 自动处理等位基因
...
cat("======================================================================\n")
cat("138 基因 MR 分析（eQTLGen 全血 - rsID 匹配）\n")
cat("======================================================================\n\n")
...
OUTPUT_DIR <- "C:/Users/Jy-Mentor-7/Desktop/你信我 这不是重蹈覆辙/mr_results_final"
...
# 步骤 2: 加载 GTEx 脑组织数据（暂不使用 - 无 rsID 无法匹配）
cat("步骤 2: GTEx 数据跳过（无 rsID 无法与 MEGASTROKE 匹配）\n")
cat("----------------------------------------------------------------------\n")
cat("  说明：GTEx v11 parquet 文件不含 rsID，只有 chr_pos_ref_alt 格式\n")
cat("        无法与 MEGASTROKE 的 rsID 匹配，需要坐标转换（超出当前范围）\n")
cat("  策略：仅使用 eQTLGen 数据（有 rsID）进行 MR 分析\n\n")
...
# 格式化 MEGASTROKE 数据为 TwoSampleMR 格式
...
for (gene_symbol in target_genes) {
  ...
  tryCatch({
    ...
    cat(sprintf("%s: ✓ 准备就绪 (%d SNPs: %d 血，F=%.2f)\n", 
                gene_symbol, nrow(exposure), n_blood, f_stat))
  }, error = function(e) {
...
# 步骤 5: MR 分析（使用 rsID 匹配 + harmonise_data）
cat("\n步骤 5: MR 分析（rsID 匹配 + harmonise_data）\n")
cat("----------------------------------------------------------------------\n")

# 格式化结局数据
outcome_data <- as.data.frame(outcome_data)  # 转换为 data.frame
outcome_data$beta.outcome <- outcome_data$hm_beta
outcome_data$se.outcome <- abs(outcome_data$hm_beta)  # 近似处理
outcome_data$effect_allele.outcome <- outcome_data$hm_effect_allele
outcome_data$other_allele.outcome <- outcome_data$hm_other_allele
outcome_data$eaf.outcome <- outcome_data$hm_effect_allele_frequency
outcome_data$pval.outcome <- 0.05  # 占位符
...
for (gene_symbol in names(prepared_genes)) {
  ...
  tryCatch({
    # 使用 harmonise_data 自动匹配 SNP
    dat <- harmonise_data(exposure_fmt, outcome_fmt)
    
    if (nrow(dat) == 0) {
      cat(sprintf("  ✗ 无匹配的 SNP\n"))
      next
    }
...
if (length(mr_results) > 0) {
  ...
  cat("5. 单源统计（仅 eQTLGen 全血）\n")
  cat("   只有血数据的基因数：", sum(results_df$n_blood > 0), "\n")
  cat("   无数据的基因数：", sum(results_df$n_blood == 0), "\n\n")
