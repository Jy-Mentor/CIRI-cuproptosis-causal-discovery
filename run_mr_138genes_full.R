#!/usr/bin/env Rscript
# ================================================================================
# 138 基因 MR 分析（完整版 - 使用宽松 eQTLGen 数据 P<1e-5）
# ================================================================================
...
cat("======================================================================\n")
cat("138 基因 MR 分析（eQTLGen P<1e-5 - rsID 匹配）\n")
cat("======================================================================\n\n")

# 配置
CLUMPED_FILE <- "D:/EQTL/clump/eQTLgen_allgene_p_1e-05_kb_1000_r2_0.001.xlsx"
OUTPUT_DIR <- "C:/Users/Jy-Mentor-7/Desktop/你信我 这不是重蹈覆辙/mr_results_138genes"
...
# 步骤 1: 加载 eQTLGen 数据
cat("步骤 1: 加载 eQTLGen 数据（P<1e-5, kb=1000, r²=0.001）\n")
cat("----------------------------------------------------------------------\n")
...
for (gene_symbol in target_genes) {
  ...
  if (is.null(ensg_id)) {
    ...
    skipped_genes <- rbind(skipped_genes, data.frame(gene = gene_symbol, reason = "无 ENSG ID", stringsAsFactors = FALSE))
    next
  }
  ...
  if (nrow(eqtlgen_snps) == 0) {
    ...
    skipped_genes <- rbind(skipped_genes, data.frame(gene = gene_symbol, reason = "无 eQTL 数据", stringsAsFactors = FALSE))
    next
  }
  ...
  if (nrow(exposure) < 2) {
    ...
    skipped_genes <- rbind(skipped_genes, data.frame(gene = gene_symbol, reason = sprintf("SNP 不足 (%d)", nrow(exposure)), stringsAsFactors = FALSE))
    next
  }
  ...
  tryCatch({
    ...
    skipped_genes <- rbind(skipped_genes, data.frame(gene = gene_symbol, reason = "格式化失败", stringsAsFactors = FALSE))
  })
}
...
if (nrow(skipped_genes) > 0) {
  write.csv(skipped_genes, file.path(OUTPUT_DIR, "skipped_genes.csv"), row.names = FALSE)
}
...
for (gene_symbol in names(prepared_genes)) {
  ...
  if (!is.na(wm_beta)) {
    direction_consistent <- sign(ivw_res$b[1]) == sign(wm_beta)
    direction_text <- ifelse(direction_consistent, "是", "否")
  } else {
    direction_text <- "NA"
  }
  ...
  result <- data.frame(
    ...
    direction_consistent = direction_text,
    stringsAsFactors = FALSE
  )
...
if (length(mr_results) > 0) {
  ...
  if (nrow(sig_results) > 0) {
    print(sig_results[, c("gene", "or", "ci_low", "ci_high", "pval", "nsnp", "f_stat", 
                          "heterogeneity_pval", "egger_pval", "direction_consistent")])
  } else {
    cat("  无显著基因 (P < 0.05)\n")
    # 显示 Top 10 最显著的基因
    cat("\nTop 10 最显著的基因:\n")
    top10 <- head(results_df, 10)
    print(top10[, c("gene", "or", "ci_low", "ci_high", "pval", "nsnp", "f_stat")])
  }
  ...
  # 保存汇总统计
  summary_results <- results_df[, c("gene", "beta", "se", "or", "ci_low", "ci_high", 
                                    "pval", "pval_adj", "f_stat", "nsnp",
                                    "heterogeneity_pval", "egger_pval", "wm_pval", 
  ...
  if (nrow(skipped_genes) > 0) {
    cat("2. 跳过原因统计\n")
    print(table(skipped_genes$reason))
    cat("\n")
  }
  ...
  cat("   F > 100 的基因数：", sum(results_df$f_stat > 100, na.rm = TRUE), "/", nrow(results_df), "\n\n")
  ...
  cat("7. 稳健性检查\n")
  cat("   方向一致的基因数：", sum(results_df$direction_consistent == "是", na.rm = TRUE), "\n\n")
  ...
  cat("9. 显著性结果\n")
  cat("   P < 0.05 的基因数：", sum(results_df$pval < 0.05, na.rm = TRUE), "\n")
  cat("   FDR < 0.1 的基因数：", sum(results_df$pval_adj < 0.1, na.rm = TRUE), "\n\n")
...
cat("分析完成！\n")
cat("======================================================================\n")
cat(sprintf("成功完成 MR 分析的基因数：%d/%d\n", success_count, length(target_genes)))
