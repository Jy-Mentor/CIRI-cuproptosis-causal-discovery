...
for (i in seq_along(target_genes)) {
  ...
  # 格式化暴露数据
  exposure_fmt <- tryCatch({
    format_data(exposure, type = "exposure")
  }, error = function(e) {
    cat(sprintf("暴露格式化失败：%s\n", e$message))
    return(NULL)
  })
  
  if (is.null(exposure_fmt)) {
    skipped <- rbind(skipped, data.frame(gene = gene_symbol, reason = "暴露格式化失败", stringsAsFactors = FALSE))
    next
  }
  ...
  if (nrow(outcome_matched) == 0) {
    cat("无匹配结局数据，跳过\n")
    failed <- rbind(failed, data.frame(gene = gene_symbol, reason = "无匹配结局 SNP", n_snps = nrow(exposure), stringsAsFactors = FALSE))
    next
  }
  ...
  outcome_fmt <- tryCatch({
    format_data(outcome_df, type = "outcome", snp_col = "hm_rsid")
  }, error = function(e) {
    cat(sprintf("结局格式化失败：%s\n", e$message))
    return(NULL)
  })
  
  if (is.null(outcome_fmt)) {
    failed <- rbind(failed, data.frame(gene = gene_symbol, reason = "结局格式化失败", n_snps = nrow(exposure), stringsAsFactors = FALSE))
    next
  }
  ...
  if (is.null(dat) || nrow(dat) == 0) {
    cat("Harmonise 失败\n")
    failed <- rbind(failed, data.frame(gene = gene_symbol, reason = "Harmonise 失败", n_snps = nrow(exposure), stringsAsFactors = FALSE))
    next
  }
  ...
  if (is.null(res) || nrow(res) == 0) {
    cat("MR 分析失败\n")
    failed <- rbind(failed, data.frame(gene = gene_symbol, reason = "MR 分析失败", n_snps = nrow(exposure), stringsAsFactors = FALSE))
    next
  }
  ...
  if (nrow(ivw_res) == 0) {
    cat("无 IVW 结果\n")
    failed <- rbind(failed, data.frame(gene = gene_symbol, reason = "无 IVW 结果", n_snps = nrow(exposure), stringsAsFactors = FALSE))
    next
  }
