...
tryCatch({
  # 安装remotes包用于从GitHub安装
  if (!require("remotes", character.only = TRUE)) {
    install.packages("remotes")
    library(remotes)
  }
  
  # 从GitHub安装TwoSampleMR
  ...
  # 方案三：使用最新版本的ieugwasr包
  cat("Updating ieugwasr to latest version...\n")
  tryCatch({
    # 安装最新版本的ieugwasr
    install.packages("ieugwasr", dependencies = TRUE)
    library(ieugwasr)
    cat("ieugwasr updated successfully!\n")
  }, error = function(e) {
    cat(sprintf("Failed to update ieugwasr: %s\n", e$message))
  })
  ...
  cat("Using provided token\n")
  
  # 使用ieugwasr的set_access_token函数设置token
  tryCatch({
    set_access_token(token)
    cat("API token set successfully!\n")
  }, error = function(e) {
    cat(sprintf("Failed to set token: %s\n", e$message))
    # 尝试其他函数名
    tryCatch({
      auth(token = token)
      cat("API token set successfully using auth()!\n")
    }, error = function(e2) {
      cat(sprintf("Failed to set token with auth(): %s\n", e2$message))
    })
  })
  ...
  tryCatch({
    test_data <- gwasinfo()
    cat(sprintf("API connection successful! Found %d datasets\n", nrow(test_data)))
  }, error = function(e) {
  ...
  stop("Cannot install necessary packages")
})
...
stroke_instruments <- extract_instruments_manual(
  outcome_id = outcome_id,
  p1 = 5e-8,  # GWAS显著性阈值
  r2 = 0.001,  # 严格LD clumping
  kb = 10000  # 10Mb
)
if(!is.null(stroke_instruments) && nrow(stroke_instruments) > 0) {
...
for(gene in genes) {
  ...
  tryCatch({
    ...
    # 方案A：先查询GTEx Brain（推荐）
    cat("Querying available eQTL datasets...\n")
    all_datasets <- gwasinfo_manual()
    
    if(!is.null(all_datasets)) {
      eqtl_datasets <- all_datasets[grep("GTEx.*brain.*cortex|eQTLGen", 
                                         all_datasets$trait, ignore.case = TRUE), ]
      
      if(nrow(eqtl_datasets) > 0) {
        # 优先使用GTEx Brain Cortex
        preferred <- eqtl_datasets[grep("cortex", eqtl_datasets$trait, ignore.case = TRUE), ]
        if(nrow(preferred) > 0) {
          eqtl_id <- preferred$id[1]
        } else {
          eqtl_id <- eqtl_datasets$id[1]
        }
        cat(sprintf("Using eQTL dataset: %s (%s)\n", eqtl_id, 
                    eqtl_datasets$trait[eqtl_datasets$id == eqtl_id]))
      } else {
        cat("No eQTL datasets found, skipping...\n")
        # 跳过该基因
        next
      }
    } else {
      cat("Failed to query eQTL datasets, skipping...\n")
      # 跳过该基因
      next
    }
    
    # 提取eQTL instruments
    exposure_dat <- extract_instruments_manual(
      outcome_id = eqtl_id,
      p1 = 5e-6,  # eQTL显著性阈值
      r2 = 0.001,  # 严格LD clumping
      kb = 10000  # 10Mb
    ...
    # 如果GTEx返回多个基因，手动筛选目标基因（如果数据包含基因名列）
    if(!is.null(exposure_dat$gene)) {
      original_count <- nrow(exposure_dat)
      exposure_dat <- exposure_dat[tolower(exposure_dat$gene) == tolower(gene), ]
      cat(sprintf("Filtered to %d SNPs specific to %s (from %d total)\n", 
                  nrow(exposure_dat), gene, original_count))
    }
    ...
    outcome_dat <- extract_outcome_data_manual(
    ...
    tryCatch({
      if(nrow(stroke_instruments) > 0 && !is.null(eqtl_id) && eqtl_id %in% all_datasets$id) {
        # 获取基因表达作为结局
        # 使用之前确定的eQTL ID
        reverse_outcome_dat <- extract_outcome_data_manual(
          snps = stroke_instruments$SNP,
          outcome_id = eqtl_id  # 使用之前确定的eQTL ID
        )
