...
OUTPUT_DIR <- "C:/Users/Jy-Mentor-7/Desktop/你信我 这不是重蹈覆辙/mr_results_matched"
...
# 2. 加载匹配后的暴露数据
# ================================================================================
cat("步骤 2: 加载匹配后的暴露数据 (双源整合)\n")
cat("----------------------------------------------------------------------\n")

load_matched_exposure <- function(exposure_dir) {
...
exposure_list <- load_matched_exposure(EXPOSURE_DIR)
...
if (nrow(results_df) > 0) {
  ...
  p_volcano <- ggplot(results_df, aes(x = log10(or), y = -log10(pval), color = fdr_sig)) +
    ...
    theme_minimal() +
    labs(
      title = "MR Analysis Results - Volcano Plot (Dual-Source Matched)",
      subtitle = paste("Matched Brain + Blood eQTL |", nrow(results_df), "genes"),
      x = "log10(Odds Ratio)",
      y = "-log10(P-value)",
      color = "FDR < 0.05"
    ) +
  ...
  p_manhattan <- ggplot(results_df, aes(x = chr_num, y = -log10(pval), color = factor(chr_num %% 2))) +
    ...
    theme_minimal() +
    labs(
      title = "MR Analysis Results - Manhattan Plot (Dual-Source)",
      subtitle = paste("Matched eQTL |", nrow(results_df), "genes"),
      x = "Gene Index",
      y = "-log10(P-value)",
      color = "Chr"
    ) +
  ...
  if (nrow(top_genes) > 0) {
    ...
    p_forest <- ggplot(top_genes, aes(x = or, y = gene)) +
      ...
      labs(
        ...
        subtitle = paste("Dual-Source Matched eQTL"),
        x = "Odds Ratio (95% CI)",
        y = "Gene",
        size = "-log10(P)"
      ) +
  ...
  p_tissue <- ggplot(tissue_contribution, aes(x = Brain, y = Blood)) +
    ...
    labs(
      ...
      subtitle = paste("Dual-Source Matched |", nrow(results_df), "genes"),
  ...
  ggsave(file.path(OUTPUT_DIR, "Figure4_Tissue_Contribution.png"), p_tissue, width = 8, height = 8, dpi = 300)
  cat(sprintf("  ✓ 已保存：Figure4_Tissue_Contribution.png\n"))
  ...
  ggsave(file.path(OUTPUT_DIR, "Figure5_F_Statistic_Distribution.png"), p_fstat, width = 10, height = 6, dpi = 300)
  cat(sprintf("  ✓ 已保存：Figure5_F_Statistic_Distribution.png\n"))
}
...
cat("  - Figure4_Tissue_Contribution.png: 双源贡献图\n")
cat("  - Figure5_F_Statistic_Distribution.png: F 统计量分布\n")
