...
# 138 基因 MR 分析（优化版）
# 参考 GitHub 权威项目最佳实践
# - 使用 ieugwasr::set_access_token() 配置 JWT
# - 使用 TwoSampleMR 标准流程
...
cat("======================================================================\n")
cat("138 基因 MR 分析（优化版 - 参考 GitHub 最佳实践）\n")
cat("======================================================================\n\n")

# 配置
EXPOSURE_DIR <- "C:/Users/Jy-Mentor-7/Desktop/你信我 这不是重蹈覆辙/exposure_matched/matched_data"
OUTPUT_DIR <- "C:/Users/Jy-Mentor-7/Desktop/你信我 这不是重蹈覆辙/mr_results_optimized"
MEGASTROKE_ID <- "ebi-a-GCST006908"  # MEGASTROKE 在 IEU 的 ID
...
# 步骤 1: 配置 JWT token 并获取 MEGASTROKE 数据
cat("步骤 1: 配置 JWT token 并获取 MEGASTROKE 数据\n")
cat("----------------------------------------------------------------------\n")

# 设置 JWT token（通过 options 设置）
options(ieugwasr.token = JWT_TOKEN)
cat("✓ JWT token 已配置\n")

# 检查可用的结局数据
tryCatch({
  available <- available_outcomes()
  megastroke <- available[available$id == MEGASTROKE_ID, ]
  
  if (nrow(megastroke) > 0) {
    cat(sprintf("✓ MEGASTROKE 研究信息:\n"))
    cat(sprintf("  ID: %s\n", megastroke$id))
    cat(sprintf("  表型：%s\n", megastroke$trait))
    cat(sprintf("  样本量：%s\n", format(megastroke$n, big.mark = ",")))
    cat(sprintf("  SNP 数量：%s\n\n", format(megastroke$nsnp, big.mark = ",")))
  }
}, error = function(e) {
  cat(sprintf("  警告：无法获取研究信息 - %s\n\n", e$message))
})
...
for (gene_symbol in names(prepared_genes)) {
  ...
  # 使用 TwoSampleMR 提取结局数据
  tryCatch({
    # 优先按 RSID 提取（传递 token 参数）
    ...
    # MR 分析（多种方法）
    res <- mr(dat, method_list = c("mr_ivw", "mr_weighted_median", "mr_egger_regressio"))
