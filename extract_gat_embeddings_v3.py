# -*- coding: utf-8 -*-
"""
GAT Embedding Extractor v3 — Dual Model (No Label Leakage) + Optimized
  ...
  v3 improvements:
    - removed dead code
    - unified gene parsing across loaders
    - fixed PPI column count check
    - per-component PCA variance reporting
    - increased epochs + neg ratio for no-DT model (anti-collapse)
    - embedding variance monitoring before/after PCA
"""
...
def main():
    print("=" * 60)
    print("  GAT Embedding v3 — Dual Blind Model (Optimized)")
    print("  Model_A: no-TD (for DG classifier)")
    print("  Model_B: no-DT (for DT classifier)")
    print("=" * 60)
    ...
    data, gene_to_idx, gene_list = build_graph(dt_genes, dg_genes, ppi_edges,
    ...
    losses_a = train_model(model_a, data_a, full_dt, None, dt_sampler_a, None,
                           n_genes, NEG_RATIO, EPOCHS_A, PATIENCE, "Model-A")

    model_a.eval()
    with torch.no_grad():
        z_a = model_a(data_a.x_dict, data_a.edge_index_dict)
        emb_a = z_a["gene"].cpu().numpy()

    # ════════════════════════════════════════
    #  MODEL B: no DT edges (for DT classifier)
    # ════════════════════════════════════════
    print("\n" + "=" * 50)
    ...
    losses_b = train_model(model_b, data_b, None, full_td, None, td_sampler_b,
                           n_genes, NEG_RATIO, EPOCHS_B, PATIENCE, "Model-B")

    model_b.eval()
    with torch.no_grad():
        z_b = model_b(data_b.x_dict, data_b.edge_index_dict)
        emb_b = z_b["gene"].cpu().numpy()
    ...
    print(f"[Raw-Emb-A] no-TD: total_var={var_raw_a:.4f}  mean_norm={mean_norm_a:.4f}")
    print(f"[Raw-Emb-B] no-DT: total_var={var_raw_b:.4f}  mean_norm={mean_norm_b:.4f}")
