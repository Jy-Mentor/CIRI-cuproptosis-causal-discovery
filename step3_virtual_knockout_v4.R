...
cat("=== 第3步：虚拟敲除干预验证（v4 - 8个BCP核心基因）===\n\n")

result_dir <- "C:/Users/Jy-Mentor-7/Desktop/你信我 这不是重蹈覆辙/causal_analysis_results"
knockout_dir <- file.path(result_dir, "virtual_knockout_results_v4")
...
# ============================================================================
# 2. 基于8个BCP基因表达筛选Top 2000细胞
# ============================================================================
cat("\n【3.2】基于8个BCP核心基因表达筛选Top 2000细胞...\n")

bcp_targets <- c("Il6", "Stat3", "Nfkb1", "Tgfb1", "Ager", "Ptgs2", "Tlr4", "Fdx1")
...
# 计算每个细胞的BCP基因覆盖度和表达总和
cell_coverage <- colSums(bcp_expr > 0)
cell_total_expr <- colSums(bcp_expr)

# 综合评分
cell_coverage_norm <- cell_coverage / length(bcp_in_data)
cell_expr_norm <- (cell_total_expr - min(cell_total_expr)) / (max(cell_total_expr) - min(cell_total_expr) + 1e-10)
...
if(length(all_degs) >= 2) {
  ...
  if(length(common_all) > 0) {
    cat("    共同基因列表:", paste(head(common_all, 20), collapse=", "), "...\n")
    write.csv(data.frame(gene=common_all), 
              file.path(knockout_dir, "common_downstream_genes_8genes.csv"), row.names=FALSE)
  }
}
...
pdf(file.path(knockout_dir, "knockout_summary_8genes.pdf"), width=16, height=10)

# 差异基因数柱状图
n_degs_list <- sapply(knockout_results, function(x) x$n_degs)
par(mar=c(5,4,4,2))
barplot(n_degs_list, main="Number of DEGs after Virtual Knockout (8 BCP genes, Top 2000 cells)",
        ylab="Number of DEGs", xlab="Knockout Gene",
        col="#2171B5", las=2, cex.names=0.8)
...
# 保存汇总结果
saveRDS(knockout_results, file.path(knockout_dir, "all_knockout_results_8genes.rds"))
